    ...
    mod.directive('sfVirtualRepeat', ['$log', '$rootElement', function ($log, $rootElement) {
        ...
        function computeRowHeight(element) {
            ...
            if (height && height !== '0px' && height !== 'auto') {
                //console.log('Row height is "%s" from css height', height);
            } else if (maxHeight && maxHeight !== '0px' && maxHeight !== 'none') {
                height = maxHeight;
                //console.log('Row height is "%s" from css max-height', height);
            } else if (element.clientHeight) {
                height = element.clientHeight + 'px';
                //console.log('Row height is "%s" from client height', height);
            } else {
        ...
        function sfVirtualRepeatCompile(element, attr, linker) {
            ...
            function sfVirtualRepeatPostLink(scope, iterStartElement, attrs) {
                ...
                function fixHeader() {
                    ...
                    var thList = $('#thdFixed').find("th")
                    if (offsetTop > 0) {
                        thList.css('top', offsetTop);
                        thList.css('position', 'relative');
                    } else {
                        thList.css('position', 'initial');
                    }
                }
                ...
                function sfVirtualRepeatOnScroll(evt) {
                    ...
                    scope.$apply(function () {
                        ...
                        //console.log('scroll to row %o', state.firstVisible);
                        sticky = evt.target.scrollTop + evt.target.clientHeight >= evt.target.scrollHeight;
                        recomputeActive();
                        //console.log(' state is now %o', state);
                        //console.log(' sticky = %o', sticky);

                        var current = sfVirtualRepeatWatchExpression(scope);
                        if (!!prev) {
                            if (angular.equals(prev, current)) {
                                fixHeader();
                            } else {
                            }
                        }
                ...
                function sfVirtualRepeatListener(newValue, oldValue, scope) {
                    ...
                    if (newValue === oldValue) {
                        ...
                        //console.log('change by %o,%o rows %s', delta, endDelta, forward ? 'forward' : 'backward');
                        if (!contiguous) {
                            //console.log('non-contiguous change');
                            ...
                            if (forward) {
                                //console.log('need to remove from the top');
                                destroyActiveElements('shift', delta);
                            } else if (delta) {
                                //console.log('need to add at the top');
                    ...
                    paddingTop = newValue.start * rowHeight;
                    dom.content.css({ 'padding-top': paddingTop + 'px' });

                    //console.log("from specific")
                    fixHeader();
